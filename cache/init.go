...
func init() {
	eject.OnAppStart(func() {
		...
		if expireStr, found := eject.Config.String("cache.expires"); found {
			var err error
		...
		if eject.Config.BoolDefault("cache.memcached", false) && eject.Config.BoolDefault("cache.redis", false) {
			panic("You've configured both memcached and redis, please only include configuration for one cache!")
		}

		// Use memcached?
		if eject.Config.BoolDefault("cache.memcached", false) {
			hosts := strings.Split(eject.Config.StringDefault("cache.hosts", ""), ",")
			if len(hosts) == 0 {
		...
		if eject.Config.BoolDefault("cache.redis", false) {
			hosts := strings.Split(eject.Config.StringDefault("cache.hosts", ""), ",")
			if len(hosts) == 0 {
			...
			password := eject.Config.StringDefault("cache.redis.password", "")
